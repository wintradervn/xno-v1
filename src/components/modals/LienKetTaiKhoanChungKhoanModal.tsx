            ...
            <>
              <div className="grid grid-cols-2 gap-x-10 gap-y-5">
                {list.map((item, index) => (
                  <div
                    key={index}
                    className="flex w-[260px] cursor-pointer items-center gap-3 rounded-[4px] p-3 text-sm font-semibold text-white hover:bg-content1"
...
function LoginForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  return (
    <form
      className="flex w-[480px] flex-col gap-5 pb-4"
      onSubmit={(e) => {
        e.preventDefault();
    ...
    >
      <div className="flex flex-col gap-2 text-sm font-semibold text-muted">
        <div>
          Tài khoản<span className="pl-1 text-red">*</span>
        </div>
        ...
        />
        <div>
          Mật khẩu <span className="pl-1 text-red"> *</span>
        </div>
        <Input
          variant="bordered"
          classNames={{
      ...
      {submitErrors && (
        <div className="-mt-2 text-sm text-red">{submitErrors}</div>
      )}
      ...
      <div className="relative my-3 h-[1px] w-full bg-neutral-500">
        <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 bg-card px-4 text-md text-muted">
          Hoặc
        </div>
      </div>
