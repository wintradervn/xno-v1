...
const NNYTDIndexBarChart = ({ symbol }: { symbol?: string }) => {
  ...
  const compiledData = useMemo(() => {
    ...
    const result = data.reduce((acc: any, item: any) => {
      ...
      acc[month].mua += item.buyForeignValue;
      acc[month].ban += item.sellForeignValue;
      acc[month].net = acc[month].mua - acc[month].ban;
      return acc;
    }, {});
...
export default React.memo(NNYTDIndexBarChart);
