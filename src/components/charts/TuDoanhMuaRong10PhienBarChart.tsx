...
export default function TuDoanhMuaRong10PhienBarChart({
  exchange,
}: {
  exchange?: string;
}) {
  const { data: heheh } = useSWR(
    `/api/index-datafeed?type=TdmbVal&from=${fromTimestamp}&to=${dayNowTimestamp}&symbol=${exchange ? exchangeMapToParam[exchange] : "VNINDEX"}`,
    async (url: string) => {
  ...
  const compiledData = useMemo(() => {
    ...
    for (let i = 0; i < heheh.t.length; i++) {
      result.push([heheh.t[i], heheh.o[i]]);
    }
          ...
          series: [
            {
              ...
              data: compiledData.map(function (item: any) {
                return {
                  ...
                  value: item[1],
                  itemStyle: {
                    color: item[1] > 0 ? "#1DF81F" : "#FF135B", // Conditional color based on value
