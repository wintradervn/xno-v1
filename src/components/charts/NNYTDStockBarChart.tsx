...
import useYTDStockData, { IYTDStockData } from "@/hooks/useYTDStockData";
echarts.use([
  CanvasRenderer,
  BarChart,
  LineChart,
...
const NNYTDStockBarChart = ({ symbol }: { symbol?: string }) => {
  const { data } = useYTDStockData(symbol || "VCB");

  const compiledData = useMemo(() => {
    if (!data) return [];
    const result = data.reduce((acc: any, item: IYTDStockData) => {
    ...
    return Object.entries(result)
      ...
      .map(([key, value]: any) => ({
        ...
        value: value.net,
        itemStyle: {
          color: value.net > 0 ? "#1DF81F" : "#FF135B", // Conditional color based on value
...
export default React.memo(NNYTDStockBarChart);
