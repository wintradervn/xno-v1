...
export default function TinHieu() {
  ...
  const tinHieu = useMemo(
    () =>
      [...tinHieuBan, ...tinHieuMua].sort((a, b) =>
        new Date(a.date) > new Date(b.date) ? -1 : 1,
      ),
    [tinHieuBan, tinHieuMua],
  );
  return (
    <div className="flex h-full w-full min-w-[200px] flex-1 flex-col">
      ...
      <ScrollArea className="py-2 pr-2">
        {tinHieu.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="flex flex-col gap-2 border-b-2 border-neutral-800 py-4 pl-2"
            >
              <div
              ...
              >
                ...
                >
                  ...
                  <div className="text-caption flex flex-shrink-0 items-center">
                    {item.MA} <FavoriteStarButton symbol={item.MA} size={16} />
                  </div>
                  {item.type === "MUA" ? (
                    <>
                      <TrendingUpSVG />{" "}
                      <div className="text-nowrap text-md font-semibold">
              ...
              <div className="text-justify text-sm text-muted">
              ...
              <div className="flex min-w-fit items-center justify-between gap-2 text-nowrap text-sm text-muted">
                <div>
                  Vùng giá:
              ...
              <div className="flex justify-between">
                ...
                <div
                  className="text-lineargreen border-lineargreen flex cursor-pointer items-center gap-1 border-b-1 text-sm hover:brightness-150"
                  onClick={() => {
                    setSelectedData(item);
                    setIsOpenDetailModal(true);
                  }}
                >
                  Xem thêm <ArrowRight className="text-[#67E1C0]" size={12} />
                </div>
              </div>
            </div>
          );
...
function DetailModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="bg-card">
      <ModalContent>
        ...
        <ModalFooter className="text-linearorange flex gap-2 text-sm">
