...
import { getLoanPackages, getPhaiSinhLoanPackages } from "@/lib/dnse-api";
...
export default function useDNSEPhaiSinhLoanPackages() {
  ...
  const { data, isLoading } = useSWR<IDNSELoanPackage[]>(
    jwtToken && accounts?.default?.id
      ? ["dnse-phai-sinh-loan-packages", jwtToken, accounts.default.id]
      : null,
    ([, token, accountId]) =>
      getPhaiSinhLoanPackages(token as string, accountId as string),
    {
