...
export default function BoLocCaNhan() {
  ...
  return (
    <div className="flex max-h-[500px] flex-col">
      <div className="flex-shrink-0 border-b-1 border-neutral-800 p-3 text-md font-semibold">
        Bộ lọc cá nhân
      </div>
      <ScrollArea className="flex-1 border-b-1 border-neutral-800 py-1">
        <div className="flex min-h-[200px] w-full flex-col items-center gap-4 p-3">
          {listBoLocCaNhan?.length ? (
            listBoLocCaNhan.map((item: any) => (
              <div
                className={cn(
                  "group relative flex h-10 w-full cursor-pointer select-none items-center rounded-[8px] border-1 p-2 px-3 text-sm font-semibold transition-all",
              ...
              >
                ...
                ></div>
                {item.name}
                <div className="absolute right-0 top-0 flex h-full items-center gap-2 pr-2 opacity-0 transition-all group-hover:opacity-100">
                  <Button
                    color="muted"
                    className="h-5 w-5 min-w-fit !bg-transparent p-0 hover:text-white"
                    onClick={() => {
                  ...
                  <Button
                    color="muted"
                    size="sm"
                    className="h-5 w-5 min-w-fit !bg-transparent p-0 text-muted hover:text-red"
                    onClick={() => {
                      setSelectingId(item.id);
        ...
        <ModalContent>
          <ModalBody className="p-5">
            ...
            <div className="flex w-full justify-stretch gap-4">
              ...
              <Button
                color="primary"
                className="flex-1 bg-red-light text-white"
                onClick={() => {
